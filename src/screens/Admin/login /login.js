  ...
  get(child(dbRef, "UsersList/" + usernameValue))
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        const userRole = userData.role; // Extract the role of the user from the database

        if (dbpass === passValue) {
          if (userRole === "admin") { // Check if the user's role is "admin"
            alert("Login successful!");
            window.location.href = "/src/screens/Admin/ManagerProduct/Admin.html";
          } else {
            alert("You must have the role 'admin' to enter.");
          }
        } else {
